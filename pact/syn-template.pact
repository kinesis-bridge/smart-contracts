...
ifdef([[_FREEZABLE_]],
  ...
  (deftable frozen-accounts:{frozen-account})

  (defcap FREEZE:bool (account:string)
    @managed
    (compose-capability (ONLY_ADMIN)))

  (defcap SEIZE:bool (account:string)
    @managed
    (compose-capability (ONLY_ADMIN)))

  (defun enforce-frozen:bool (account:string)
    (with-default-read frozen-accounts account {'frozen:false} {'frozen:=frozen}
      (enforce frozen (format "Account {} not frozen" [account]))))

  (defun enforce-not-frozen:bool (account:string)
    (with-default-read frozen-accounts account {'frozen:false} {'frozen:=frozen}
      (enforce (not frozen) (format "Account {} frozen" [account]))))

  (defun freeze-account:string (account:string)
    (with-capability (FREEZE account)
      (write frozen-accounts account {'frozen:true})))

  (defun seize-account:string (seized-account:string dst-account:string)
    (with-capability (SEIZE seized-account)
      (enforce-frozen seized-account)
      (with-read accounts seized-account {'balance:=seized-balance}
        (with-read accounts dst-account {'balance:=dst-balance}
...
(cond
  ((read-msg "init") [ (create-table NAMESPACE.SYMBOL.accounts)
                       ...
                       (create-table NAMESPACE.SYMBOL.throttling-table)
                       (init-throttling)
                       "Initialized"
                      ])
