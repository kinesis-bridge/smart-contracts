...
(module hyp-erc20-collateral GOVERNANCE
  ...
  (defun ramp:decimal (x:decimal)
    @doc "Return 0 if negative, x otherwise"
    (if (< x 0.0) 0.0 x))
...
(cond
  ((read-msg "init") [ (create-table NAMESPACE.hyp-erc20-collateral.accounts)
                       (create-table NAMESPACE.hyp-erc20-collateral.contract-state)
                       (create-table NAMESPACE.hyp-erc20-collateral.routers)
                       (create-table NAMESPACE.hyp-erc20-collateral.throttling-table)
                       (init-throttling)
                       "Initialized"])

  ((read-msg "upgrade-to-v2") [ (create-table NAMESPACE.hyp-erc20-collateral.throttling-table)
                                (init-throttling)
                                "Upgraded-to-v2"])
  "Upgrade complete")